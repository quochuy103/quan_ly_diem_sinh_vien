...
const GradeStatistics = () => {
  ...
  const currentData = selectedPeriod === "weekly" ? mockStatistics.weekly : mockStatistics.monthly;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Thống kê và phân tích điểm số sinh viên</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Điểm TB hiện tại</CardTitle>
            {trend === "up" ? 
          ...
          <CardContent>
            ...
            <p className={`text-xs ${trend === "up" ? "text-green-600" : "text-red-600"}`}>
              {trend === "up" ? "+" : "-"}{trendValue} so với kỳ trước
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{currentData[currentData.length - 1]?.passRate || 0}%</div>
            <p className="text-xs text-muted-foreground">
              {45 - Math.floor(45 * (currentData[currentData.length - 1]?.passRate || 0) / 100)} sinh viên trượt môn
            </p>
          </CardContent>
        </Card>
              ...
              {mockStatistics.gradeDistribution.map((item, index) => (
                <div key={index} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm font-medium">{item.range} điểm</span>
                  </div>
                  <div className="flex items-center gap-2">
